  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 py-10 bg-mesh pb-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex bg-slate-900/80 p-1.5 rounded-2xl border border-slate-800 max-w-md">
          <button
            ...
            className={`flex-1 py-3 text-xs font-bold rounded-xl transition flex items-center justify-center space-x-2 ${
              activeTab === 'flights' ? 'bg-cyan-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-slate-100'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-3 text-xs font-bold rounded-xl transition flex items-center justify-center space-x-2 ${
              activeTab === 'hotels' ? 'bg-sky-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-slate-100'
            }`}
          >
        ...
        {activeTab === 'flights' && (
          <div className="glass-panel p-5 rounded-3xl border border-slate-800 flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[140px]">
              <label className="block text-[11px] font-bold text-slate-400 mb-1">Departure Airport</label>
              <input
                type="text"
                value={origin}
            ...
            <div className="flex-1 min-w-[140px]">
              <label className="block text-[11px] font-bold text-slate-400 mb-1">Destination Airport</label>
              <input
                type="text"
            ...
            <div className="flex-1 min-w-[160px]">
              <label className="block text-[11px] font-bold text-slate-400 mb-1">Flight Date</label>
              <input
                type="date"
                value={date}
            ...
            <button
              ...
              className="px-6 py-3 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-black rounded-xl text-xs flex items-center space-x-2 shadow-lg shadow-cyan-500/20 transition"
            >
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="glass-panel p-12 rounded-3xl text-center space-y-3">
                ...
                <h4 className="text-base font-bold text-slate-200">No flights found for this route</h4>
                <p className="text-xs text-slate-400">Try searching from JFK, SFO, or LHR to CDG, HND, or DPS.</p>
              </div>
            ) : (
              flights.map((f, i) => (
                <div
                  key={i}
                  className="glass-panel p-6 rounded-3xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-cyan-500/50 transition"
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-base font-bold text-slate-100">{f.airline}</span>
                  ...
                  <div className="flex items-center space-x-6">
                    <div className="text-right">
                      ...
                      <div className="text-2xl font-black text-emerald-400">${f.price}</div>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {loading ? (
              ...
              hotels.map((h, i) => (
                <div
                  key={i}
                  className="glass-panel p-5 rounded-3xl border border-slate-800 space-y-4 hover:border-sky-500/50 transition"
                >
                  <img src={h.imageUrl} alt={h.name} className="w-full h-48 object-cover rounded-2xl" />
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="text-lg font-bold text-slate-100 font-heading">{h.name}</h4>
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-slate-800">
                    <div>
                      <span className="text-[10px] text-slate-500 uppercase block font-bold">Price / Night</span>
                      <span className="text-xl font-black text-emerald-400">${h.pricePerNight}</span>
                    </div>
                    <button
                      onClick={() => setSelectedBooking(h)}
                      className="px-5 py-2.5 bg-sky-500 hover:bg-sky-400 text-slate-950 font-extrabold rounded-xl text-xs transition"
                    >
                      Reserve Room
                    </button>
                  </div>
          ...
          <div className="glass-panel w-full max-w-md rounded-3xl p-6 border border-slate-700/60 shadow-2xl relative">
            ...
            {bookingSuccess ? (
              <div className="text-center py-8 space-y-3">
                ...
                <h3 className="text-2xl font-black text-slate-100 font-heading">Reservation Confirmed!</h3>
              ...
              <div className="space-y-6">
                <div className="flex items-center space-x-3 text-cyan-400">
                  ...
                  <h3 className="text-xl font-black font-heading text-slate-100">Stripe Instant Checkout</h3>
                </div>

                <div className="bg-slate-950/80 p-4 rounded-2xl border border-slate-800 space-y-2 text-xs">
                  <div className="flex justify-between text-slate-300">
                    <span>Booking item:</span>
                    <span className="font-bold text-slate-100">{selectedBooking.airline || selectedBooking.name}</span>
                  </div>
                  <div className="flex justify-between text-slate-300">
                ...
                <div className="space-y-3">
                  <input
                    ...
                    className="w-full glass-input text-xs text-slate-400"
                  />
                  <div className="flex space-x-2">
                    <input type="text" disabled value="12/28" className="w-1/2 glass-input text-xs text-slate-400" />
                    <input type="text" disabled value="888" className="w-1/2 glass-input text-xs text-slate-400" />
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full py-3.5 bg-gradient-to-r from-emerald-400 to-cyan-500 text-slate-950 font-black rounded-xl text-sm shadow-xl shadow-emerald-500/20 transition flex items-center justify-center space-x-2"
                >
                  {loading ? (
